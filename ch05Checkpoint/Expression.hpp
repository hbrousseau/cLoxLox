...
class literalExpr : public Expr, public std::enable_shared_from_this<literalExpr> {
    public:
        ...
        std::string toString() const {
            ...
            else if (value.type() == typeid(std::string)) return std::any_cast<std::string>(value);
            else if (value.type() == typeid(double)) return std::to_string(std::any_cast<double>(value)); 
            else if (value.type() == typeid(bool)) return std::any_cast<bool>(value) ? "true" : "false";
            // else we have an error
            return "Error in literalVisitor: literal type is not recognized.";
        }
};
